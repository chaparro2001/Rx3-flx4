...
pkill -x rbp-pi; pkill -f "^python3 /home/rx3/rx3-handoff/flx4-bridge"; pkill -x rx3-fb-present; pkill -f "^/home/rx3/rx3-touch-bridge"
for i in 1 2 3 4 5; do pgrep -x rbp-pi >/dev/null || break; sleep 1; done
pgrep -x rbp-pi >/dev/null && pkill -9 -x rbp-pi
systemctl stop rx3-priv.service 2>/dev/null
for p in usb1 usb2; do
  for mp in $R/media/$p/*; do mountpoint -q "$mp" && umount -l "$mp"; done
  mountpoint -q /home/rx3/rx3-usb/$p/lower && umount -l /home/rx3/rx3-usb/$p/lower
done
rm -f $R/dev/sd??; /home/rx3/rx3-handoff/rx3-mtab.sh
