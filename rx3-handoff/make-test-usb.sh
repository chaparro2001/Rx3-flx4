...
set -e
IMG=/home/rx3/rx3-test-usb.img
python3 - <<'PY'
import struct,math,wave,os
os.makedirs('/home/rx3/test-tracks',exist_ok=True)
for name,freq,secs in (('Tone A 440',440,60),('Tone B 660',660,60)):
    w=wave.open('/home/rx3/test-tracks/%s.wav'%name,'wb'); w.setnchannels(2); w.setsampwidth(2); w.setframerate(44100)
    frames=bytearray()
...
mkdir -p /mnt/rx3-img && mount -o loop $IMG /mnt/rx3-img && mkdir -p "/mnt/rx3-img/Contents/Test Album" && cp /home/rx3/test-tracks/*.wav "/mnt/rx3-img/Contents/Test Album/" && umount /mnt/rx3-img
