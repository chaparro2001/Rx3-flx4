...
usage: rx3-tap.py x y [hold_seconds]   (landscape presenter layout: content at canvas x=160, scale 1.25)"""
import subprocess, struct, time, sys
x, y = int(sys.argv[1]), int(sys.argv[2]); hold = float(sys.argv[3]) if len(sys.argv) > 3 else 0.25
cx, cy = 160 + x * 1600 // 1280, y * 1000 // 800
p = subprocess.Popen(['/home/rx3/rx3-touch-bridge', '--replay', '/home/rx3/rx3-rootfs/dev/tsc2007_2-0048'], stdin=subprocess.PIPE, stderr=subprocess.DEVNULL)
